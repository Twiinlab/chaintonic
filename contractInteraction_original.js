...
const solc = require('./config/solcV0.3.6');
const fs = require('fs');


const accounts = require('./config/accounts.js').accounts;
const erisdbURL = require('./config/erisdbURL.js').erisdbURL;
...
initialize = function(){
    ...
    contract = fs.readFileSync('./contracts/PolicyV0.3.X.sol', 'utf-8');
    if(contract){
        compiledContract = solc.compile(contract, 1).contracts.Policy;
